...
module.exports = {
  ...
  run: async function (toolbox: GluegunToolbox) {
    ...
    table([
      ...
      [
        column1('directory'),
        column2(directory.split(separator).pop()),
        column3(directory),
      ],
    ]);
    ...
    const nodeVersion = (await run('node --version', { trim: true })).replace(
      'v',
      '',
    );
    ...
    const yarnVersion =
      yarnPath && (await run('yarn --version', { trim: true }));

    const nodeInfo = [column1('node'), column2(nodeVersion), column3(nodePath)];
    const npmInfo = [
      column1('npm'),
      column2(npmVersion),
      column3(npmPath || 'not installed'),
    ];
    const yarnInfo = [
      column1('yarn'),
      column2(yarnVersion),
      column3(yarnPath || 'not installed'),
    ];

    async function packageInfo(npmOrYarn: 'npm' | 'yarn') {
      return (await packager.list({ packager: npmOrYarn, global: true })).map(
        (nameAndVersion) => [
    ...
    const haveGlobalPackages =
      npmPackages.length > 0 || yarnPackages.length > 0;

    info('');
    info(
      colors.cyan(
        `JavaScript${
          haveGlobalPackages ? ' (and globally-installed packages)' : ''
        }`,
      ),
    );
    ...
    const javaVersion =
      javaPath && (await run(javaVersionCmd))?.match(/"(.*)"/)?.slice(-1)[0];
    ...
    if (isMac) {
      ...
      const xcodeVersion =
        xcodePath &&
        (await run('xcodebuild -version', { trim: true })).split(/\s/)[1];
      ...
      table([
        [
          column1('cocoapods'),
          column2(cocoaPodsVersion),
          column3(cocoaPodsPath),
        ],
      ]);
    }
